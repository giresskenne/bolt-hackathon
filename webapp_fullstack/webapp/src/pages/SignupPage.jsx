...
export default function SignupPage() {
  ...
  const { signup, signupWithGoogle, isLoading, isAuthenticated, user } = useAuthStore()
  ...
  const handleGoogleSignup = async () => {
    console.log('🟦 Google SIGNUP button clicked (new users only)!');
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast.loading('Redirecting to Google for signup...')
      } else {
        showToast.error(result.error || 'Google signup failed. If you already have an account, please sign in.', {
          title: 'Google Signup Failed'
        })
      }
    } catch (error) {
      showToast.error('Unable to connect to Google. If you have an account, try signing in instead.', {
          ...
          <div className="bg-white/5 backdrop-blur-lg rounded-3xl p-8 border border-white/10">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {/* Add note about new users only */}
                <p className="text-xs text-gray-500 text-center">
                  Google sign-up is for new users only. {" "}
