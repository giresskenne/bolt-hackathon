/*─────────────────────────────────────────────────────────────
  Privly-Scrubber – contentScript.js  (v0.3.0)
...
function getThemeAwareBackground() {
  ...
  console.log('[Scrubber] Theme detection:', {
    ...
    isDarkMode
      ...
      // Site-specific overrides for problematic sites
  const hostname = window.location.hostname;
  let finalIsDarkMode = isDarkMode;

  // For Perplexity AI, ignore DOM classes and use system preference only
  // (their DOM has dark classes even in light mode)
  if (hostname.includes('perplexity.ai')) {
    finalIsDarkMode = prefersDark; // Use system preference instead
    console.log('[Scrubber] Perplexity.ai detected - using system preference:', prefersDark ? 'dark' : 'light');
  }
  ...
  if (finalIsDarkMode) {
    ...
    return 'linear-gradient(90deg, #fff 0%, rgb(244, 185, 184) 66.66%, rgb(244, 185, 184) 100%)';
  }
}
...
function setupThemeListener() {
  ...
  // Function to set up DOM observers
  const setupDOMObservers = () => {
    // Check if elements exist before observing
    if (!document.body || !document.documentElement) {
      console.warn('[Scrubber] DOM elements not ready, retrying...');
      return false;
    }
    
    // Listen for class changes on body/html that might indicate theme changes
  ...
  // Try to set up observers immediately, or wait for DOM
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupDOMObservers);
  } else {
    // DOM is already loaded
    if (!setupDOMObservers()) {
      // If still failed, try again after a short delay
